...
class BattWarsCustomObject(object):
    def __init__(self, obj):
        self._attributes = {}

        self.type = obj.get("type")
        self.id = obj.get("id")
        #self.pos = number_count
        self._xml_node = obj
...
class BattWarsObject(object):
    ...
    def get_all_attr(self):
        return [name for name in self._attributes]

    def add_attr(self):
        pass

    #def get_attrs(self, name):
    #    return [self._attributes[name] in self.attributes[name]]
    ...
    def set_attr_values(self, name, val, pos=0):
        self._attributes[name][pos].text = val
...
class BattWarsLevel(object):
    ...
    def add_attr(self, xml_node):
        #bwobj = BattWarsObject(xml_node)
        #sub = self._root(self._root, "Attribute")
        root = etree.Element(xml_node)
        print(etree.tostring(root))
        #ro = self._root("Summ")
        #ro.set("status", "Completed")
        #self._root.insert(0, xml_node)
        #bwobj.set_attr_value(val,num)
    ...
    def custom_add_object(self,xml_node,val,num,obj):
        bwobj = BattWarsCustomObject(xml_node)
        bwobj.set_attr_value(val,num)
        self.custom_add.append(xml_node)
        self.custom_pos.append(obj.pos)

    def custom_finished(self):
        y = 0
        for x in self.custom_add:
            bwobj = BattWarsObject(x)
            assert bwobj.id not in self.obj_map
            self._root.insert(self.custom_pos[y]+1,x)
            self.obj_map[bwobj.id] = bwobj
            y += 1
        self.custom_add = []
        self.custom_pos = []
