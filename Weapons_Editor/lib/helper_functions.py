...
def entity_set_val(xml, entityid, to_fix,to_fix2):
    if entityid != "":
        entityobj = xml.obj_map[entityid]
        try:
            done = entityobj.get_attr_value(to_fix)
            entityobj.set_attr_value(to_fix,to_fix2)
        except:
            #traceback.print_exc()
            return None
    else:
...
def entity_set_list(xml, entityid, to_fix, to_fix2, to_fix3):
    if entityid != "":
        obj = xml.obj_map[entityid]
        matrix = [x for x in obj.get_attr_value(to_fix).split(",")]
        print(matrix)
        try:
            matrix = [x for x in obj.get_attr_value(to_fix).split(",")]
            matrix[to_fix3] = to_fix2
            mat_fin = ','.join(matrix)
            obj.set_attr_value(to_fix,mat_fin)
            print("finsihed")
        except:
            print("nope")
            return None
...
def entity_set_list2(xml, entityid, to_fix, to_fix2, to_fix3):
    try:
        if entityid != "":
            obj = xml.obj_map[entityid]
            matrix = obj.get_attr_elements(to_fix)
            obj.set_attr_value(to_fix, to_fix2,to_fix3)
        else:
            return None
    except:
        print("nope")
